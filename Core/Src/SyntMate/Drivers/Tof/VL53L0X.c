...
uint8_t _I2CWrite( uint8_t *pdata, uint32_t count)
{
uint32_t i2c_time_out = I2C_TIME_OUT_BASE+ count* I2C_TIME_OUT_BYTE;
	return (uint8_t)HAL_I2C_Master_Transmit(&TOF_I2C, TOF_ADDRESS, pdata, count, i2c_time_out);
}

uint8_t _I2CRead( uint8_t *pdata, uint32_t count)
{
uint32_t	i2c_time_out = I2C_TIME_OUT_BASE+ count* I2C_TIME_OUT_BYTE;
	return (uint8_t)HAL_I2C_Master_Receive(&TOF_I2C, TOF_ADDRESS|1, pdata, count, i2c_time_out);
}
...
uint32_t VL53L0X_init(uint32_t io_2v8)
{
...
uint32_t VL53L0X_writeReg(uint8_t reg, uint8_t value)
{
	_I2CBuffer[0] = reg;
	_I2CBuffer[1] = value;

	if (_I2CWrite(_I2CBuffer, 2) != 0)
		return 1;
	return 0;
}

uint32_t VL53L0X_writeReg16Bit(uint8_t reg, uint16_t value)
{
	_I2CBuffer[0] = reg;
	_I2CBuffer[1] = value >> 8;
	_I2CBuffer[2] = value & 0xff;

	if (_I2CWrite(_I2CBuffer, 3) != 0)
		return 1;
	return 0;
}

uint32_t VL53L0X_writeReg32Bit(uint8_t reg, uint32_t value)
{
	_I2CBuffer[0] = reg;
	_I2CBuffer[1] = value >> 24;
	_I2CBuffer[2] = value >> 16;
	_I2CBuffer[3] = value >> 8;
	_I2CBuffer[4] = value & 0xff;

	if (_I2CWrite(_I2CBuffer, 5) != 0)
		return 1;
	return 0;
}
...
uint8_t data;
	_I2CBuffer[0] = reg;
	if ( _I2CWrite(_I2CBuffer, 1) != 0 )
		return 1;
	if ( _I2CRead(&data, 1) != 0 )
		return 1;
	return data;
}
...
uint32_t VL53L0X_writeMulti(uint8_t reg, uint8_t *src, uint8_t count)
{
	if (_I2CWrite(src, count) != 0)
		return 1;
	return 0;
}
...
uint32_t VL53L0X_readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
	_I2CBuffer[0] = reg;
	if ( _I2CWrite(_I2CBuffer, 1) != 0 )
		return 1;
	if ( _I2CRead(dst, count) != 0 )
		return 1;
	return 0;
}
...
void VL53L0X_StartReadRangeSingleIRQ(void)
{
	VL53L0X_writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);
	VL53L0X_writeReg(0x80, 0x01);
	VL53L0X_writeReg(0xFF, 0x01);
...
uint16_t range,tof_rev_id,status,measure_in_progress,measure_number;

void SyntMate_VL53L0X_Init(void)
{
	HAL_GPIO_WritePin(VL53L0X_RST_GPIO_Port, VL53L0X_RST_Pin, GPIO_PIN_RESET);
	HAL_Delay(2);
	HAL_GPIO_WritePin(VL53L0X_RST_GPIO_Port, VL53L0X_RST_Pin, GPIO_PIN_SET);
	HAL_Delay(2);
	tof_rev_id = VL53L0X_readReg(IDENTIFICATION_REVISION_ID);
	status = VL53L0X_init(0);
	VL53L0X_setTimeout(500);
}

void SyntMate_VL53L0X_StartMeasure(void)
{
	measure_in_progress=1;
	measure_number=NUM_MEASURES;
	range = 0;
	VL53L0X_StartReadRangeSingleIRQ();
}

void VL53L0X_ReadRangeSingleIRQ_Callback(void)
{
	VL53L0X_writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);
